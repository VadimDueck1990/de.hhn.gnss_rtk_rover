...
class NMEAReader:
    ...
    def __init__(self, i2c, uart, **kwargs):
        """Constructor.
        ...
        if msgmode not in (0, 1, 2):
            raise nme.NMEAStreamError(
                "Invalid stream mode - must be 0, 1 or 2."
            )
    ...
    def __next__(self) -> "tuple[bytes, NMEAMessage]":
        """
        ...
        (raw_data, parsed_data) = self.read_i2c()
    ...
    def read_i2c(self) -> tuple:
        ...
        while reading:  # loop until end of valid NMEA message or EOF
            byte1 = self._i2c.readfrom_mem(0x42, 0xFF, 1) # read 1st byte
            if len(byte1) < 1: #EOF
                break
            if byte1 != b"\x24":  # not NMEA, discard and continue
                continue
            byte2 = self._i2c.readfrom_mem(0x42, 0xFF, 1)  # read 2nd byte to confirm protocol
            if len(byte2) < 1:  # EOF
                break
            bytehdr = byte1 + byte2
            
            if bytehdr in NMEA_HDR:  # it's a NMEA message
                byte_payload = self._i2c.readfrom_mem(0x42, 0xFF, 2)
                while byte_payload[-2:] != b"\x0d\x0a": # NMEA protocol is CRLF terminated
                    byte_payload += self._i2c.readfrom_mem(0x42, 0xFF, 1)
    ...
    async def read_uart(self) -> tuple:
        """
        Read the binary data asynchronously from uart.
    ...
    def parse(message: bytes, **kwargs) -> object:
        ...
        if msgmode not in (0, 1, 2):
            raise nme.NMEAParseError(
                "Invalid parse mode - must be 0, 1 or 2."
            )
        try:
