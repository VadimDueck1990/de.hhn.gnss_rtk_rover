...
from pynmeagps import nmeatypes_core as nmt
from pynmeagps import nmeatypes_get as nmg
from pynmeagps import nmeatypes_poll as nmp
from pynmeagps import nmeatypes_set as nms
...
#             ) from err
    ...
    def _get_dict(self, **kwargs):
        ...
        :return: ordered dictionary representing payload definition
        :rtype: OrderedDict
        """
        ...
        try:
            ...
            final_dict = nmg.NMEA_PAYLOADS_GET[key]
            print("MsgID: " + str(key))
#             print("Dict: " + str(final_dict))
#             for dkey in final_dict:
#                 print(dkey, final_dict[dkey])
            return final_dict
...
#            f"{self._mode}, payload={self._payload})"
    ...
    def val2str(val, att: str, hpmode: bool = False) -> str:
        ...
        if att in (nmt.CH, nmt.ST):
            ...
            print("Attr: " + str(att) + " Value: " + str(val))
            vals = time2str(val)
        elif att == nmt.DT:
            vals = date2str(val)
        else:
