...
from utils.udatetime import Udate, Utime
...
def date2utc(dates: str) -> Udate:
    """
    Convert NMEA Date to Date.

    :param str dates: NMEA date ddmmyy
    :return: Udate
    :rtype: udatetime.Udate
    """

    try:
        date = Udate.dateFromNmeaString(dates)
        return date
    except (TypeError, ValueError):
        return ""


def time2utc(times: str) -> Utime:
    """
    Convert NMEA Time to Utime.

    :param str times: NMEA time hhmmss.ss
    :return: Utime
    :rtype: udatetime.Utime
    """

    try:
        time = Utime.timeFromNmeaString(times)
        return time
    except (TypeError, ValueError):
        return ""


def time2str(tim: Utime) -> str:
    """
    Convert udatetime.Utime to NMEA formatted string.

    :param udatetime.Utime tim: time
    :return: NMEA formatted time string hhmmss.ss
    :rtype: str
    """

    try:
        time = tim.nmeaStringFromUtime()
        return time
    except (AttributeError, TypeError, ValueError):
        return ""


def date2str(dat: Udate) -> str:
    """
    Convert udatetime.Udate to NMEA formatted string.

    :param udatetime.Udate dat: date
    :return: NMEA formatted date string ddmmyy
    :rtype: str
    """

    try:
        date = dat.nmeaStringFromUdate()
        return date
    except (AttributeError, TypeError, ValueError):
        return ""
